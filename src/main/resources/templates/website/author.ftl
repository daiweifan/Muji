	...
	<section class="section-block">
		<div id="fh5co-intro-section">
			<div class="container">
				...
				<input type="hidden" id="backgroundImage" value="${author.titleImage!}"/>
					<div class="col-sm-4 text-center block-intro-left">
						<h4>${author.name!}</h4>
						<p>${author.position!}</p>
						<img src="${author.avatar}?imageView2/1/w/200/h/200" class="img-circle" >
					</div>
				...
				<div class="row masonry" id="list_hook">
					<#list cases as case>
						<div class="col-sm-4 col-md-4 masonry_item">
							<a href="/case?id=${case.id}">
								<img src="/img/loading.gif" alt="" class="img-responsive img_lazyload" data-original="${case.coverImage!}"  >
							</a>
						</div>
					</#list>
	...
	<section class="section-block">
		...
		<div class="row">
			<#if randList?? && (randList?size > 0)>
              <#list randList as author>
                 <div class="col-xs-12 col-sm-4 col-md-4  mt-20">
					<h4>${author.name!}</h4>
					<div class="img">
						<a href="/author?id=${author.id!}" class="expand">
							<img src="${author.avatar!}" alt="案例详情" class="img-responsive" style="width:300px;height:200px;">
						</a>
                    </div>
				</div>
             </#list>
